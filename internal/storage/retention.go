...
// retained ambient history. Cascades packet_observations and checkin_packets.
// retainWeeks: 0 keeps only checkin-linked raw rows (no extra traffic).
func (s *SQLiteStore) PruneRawPackets(ctx context.Context, now time.Time, tz string, retainWeeks int, dayOfWeek time.Weekday) (deleted int64, err error) {
	cutoffWeekStart := checkins.WeekStartForWeekday(now, tz, dayOfWeek).AddDate(0, 0, -7*retainWeeks)
	...
	for {
		...
		for _, r := range rows {
			if s.shouldRetainRawPacket(r.packetHash, r.observedAt, tz, dayOfWeek, cutoffWeekStart, protected) {
				continue
			}
...
func (s *SQLiteStore) shouldRetainRawPacket(packetHash string, observedAt time.Time, tz string, dayOfWeek time.Weekday, cutoffWeekStart time.Time, protected map[string]struct{}) bool {
	if _, ok := protected[packetHash]; ok {
		return true
	}
	if !checkins.IsWeekdayInTZ(observedAt, tz, dayOfWeek) {
		return false
	}
	ws := checkins.WeekStartForWeekday(observedAt, tz, dayOfWeek)
